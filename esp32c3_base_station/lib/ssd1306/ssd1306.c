...
// Thêm vào cuối file ssd1306.c

esp_err_t ssd1306_fill_space(uint8_t x1, uint8_t x2, uint8_t y1, uint8_t y2, bool fill)
{
    return i2c_ssd1306_buffer_fill_space(&i2c_ssd1306, x1, x2, y1, y2, fill);
}

esp_err_t ssd1306_draw_image(uint8_t x, uint8_t y, const uint8_t *image, uint8_t img_width, uint8_t img_height, bool invert)
{
    return i2c_ssd1306_buffer_image(&i2c_ssd1306, x, y, image, img_width, img_height, invert);
}